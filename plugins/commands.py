...
from utils import get_settings, pub_is_subscribed, get_size, is_subscribed, save_group_settings, temp, verify_user, check_token, check_verification, get_token, get_shortlink, get_tutorial, get_seconds, generate_referral_buttons
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help"]:
        if PREMIUM_AND_REFERAL_MODE == True:
            ...
            buttons = [[
                ...
                InlineKeyboardButton('✇ Jᴏɪɴ Uᴘᴅᴀᴛᴇs ✇', url=CHNL_LNK)
            ]]
        reply_markup = InlineKeyboardMarkup(buttons)      
        await message.reply_photo(
            photo=random.choice(PICS),
...
# Delete the log message
        ...
        if is_valid == True:
           # await message.reply_text(text=f"<b>ʜᴇʏ {user} 😍 ,You are successfully verified !\nNow you have unlimited access for all movies for 1 week ✅.</b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"https://t.me/+K9wdpqiSRXQ2NWY1")]]))
